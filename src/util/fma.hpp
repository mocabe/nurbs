...
namespace nurbs::detail {
  ...
  template <class FP>
  constexpr bool
      fast_fma_enabled = (std::is_same_v<double, FP> && fp_fast_fma) ||
                         (std::is_same_v<float, FP> && fp_fast_fmaf) ||
                         (std::is_same_v<long double, FP> && fp_fast_fmal);
}
...
namespace nurbs::detail{
  ...
  struct fma_division_impl {
    static void fma_division(FP a, const V& x, const V& y, V& result) {
      auto yval = get<N - 1>(y);
      auto xval = get<N - 1>(x);
      // Some compilers don't support FMA as library but still able to auto-generate it.
...
namespace nurbs {
  ...
  void fma_division(FP a, const V& x, const V& y, V& result){
    detail::fma_division_impl<dimension_v<V>,std::decay_t<FP>,std::decay_t<V>>::fma_division(a, x, y, result);
  };
}